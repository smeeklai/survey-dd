...
questionnaire = ['type of uses', 'energy demand', 'diesel price', 'do you love energy']
registering_question = ['เพศ', 'วัน/เดือน/ปีเกิด', 'ที่อยู่', 'จำนวนคนในบ้าน']
tip_table = dynamodb.Table('tips')
    ...
    if(event.message.text != 'ยัง'):
        if (registered == False):
            ...
            if(during_survey == True or event.message.text == 'เริ่ม'):
                ...
                if(len(question)!=0):
                    during_survey = True
                    messages = [question.pop(0)['question']]
                    if (len(question) == 0):
                        done = True
...
def get_questionnaire(userId):
    ...
    # print(question)
    # & Key('target-audience-age-min').lte(user_profile['age'])
    return question
